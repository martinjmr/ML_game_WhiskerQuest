...
class DQN(Agent):
    ...
    def reinforce(self, s_, n_s_, a_, r_, game_over_):
        ...
        s_ = s_.to(device)
        n_s_ = n_s_.to(device)
        a_ = torch.tensor(a_, device=device).long()
        r_ = torch.tensor(r_, device=device).float()
        game_over_ = torch.tensor(game_over_, device=device).bool()
        ...
        loss = criterion(self.model(input_states), target_q.detach())
